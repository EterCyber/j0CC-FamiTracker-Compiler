...
const CString& CTextExport::ExportCellText(const stChanNote& stCell, unsigned int nEffects, bool bNoise)		// // //
{
	...
	static const TCHAR* TEXT_NOTE[ECHO+1] = {		// // //
		_T("..."),
		_T("C-?"), _T("C#?"), _T("D-?"), _T("D#?"), _T("E-?"), _T("F-?"),
		_T("F#?"), _T("G-?"), _T("G#?"), _T("A-?"), _T("A#?"), _T("B-?"),
		_T("==="), _T("---"), _T("^-?") };
		
	s = (stCell.Note <= ECHO) ? TEXT_NOTE[stCell.Note] : _T("...");		// // //
...
const CString& CTextExport::ExportFile(LPCTSTR FileName, CFamiTrackerDoc *pDoc)
{
	...
	s.Format(_T("# Song information\n"
	            ...
	            "%-15s %s\n"
	            "\n"),
	            CT[CT_TITLE],     ExportString(CString(pDoc->GetSongName())),
	            CT[CT_AUTHOR],    ExportString(CString(pDoc->GetSongArtist())),
	            CT[CT_COPYRIGHT], ExportString(CString(pDoc->GetSongCopyright())));
	f.WriteString(s);
...
//				,CT[CT_PLAYBACKRATE], pDoc->, pDoc->
	...
	for (int smp=0; smp < MAX_DSAMPLES; ++smp)
	{
		if (const CDSample* pSample = pDoc->GetSample(smp))		// // //
		{
			...
			s.Format(_T("%s %3d %5d %s\n"),
				...
				smp,
				size,
				ExportString(CString(pSample->GetName())));
			f.WriteString(s);
	...
	for (int i = 0; i < MAX_GROOVE; i++) {
		...
		if (Groove != NULL) {
			...
			for (int j = 0; j < Groove->GetSize(); j++) {
				s.Format(_T(" %d"), Groove->GetEntry(j));
				f.WriteString(s);
			}
			f.WriteString(_T("\n"));
		}
	...
	for (unsigned int i=0; i<MAX_INSTRUMENTS; ++i)
	{
		...
		f.WriteString(ExportString(CString(pInst->GetName())));
		f.WriteString(_T("\n"));
	...
	for (unsigned int t=0; t < pDoc->GetTrackCount(); ++t)
	{
		const char* zpTitle = CT2A(pDoc->GetTrackTitle(t).GetString());
