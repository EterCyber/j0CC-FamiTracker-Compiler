...
CSoundGen *CSoundGen::GetObject()
{
	static CSoundGen Object;
	return &Object;
}
...
bool CSoundGen::ResetAudioDevice()
{
	...
	CSettings *pSettings = CSettings::GetObject();
...
void CSoundGen::BeginPlayer(play_mode_t Mode, int Track)
{
	...
	if (!m_pDocument || !m_pDocument->IsFileLoaded())
		return;
...
float CSoundGen::GetCurrentBPM() const		// // //
{
	...
	float BPM = std::min(GetAverageBPM(),
						 EngineSpeed * 15);		// // // 050B
...
void CSoundGen::QueueNote(int Channel, stChanNote &NoteData, note_prio_t Priority) const
{
	...
	CMIDI::GetObject()->WriteNote(Channel, NoteData.Note, NoteData.Octave, NoteData.Vol);
}
