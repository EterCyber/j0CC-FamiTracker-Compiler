	...
	m_pBookmarkManager(new CBookmarkManager(MAX_TRACKS))
{
	...
	CSoundGen *pSoundGen = CSoundGen::GetObject();
...
BOOL CFamiTrackerDoc::OnSaveDocument(LPCTSTR lpszPathName)
{
	...
	if ((m_bForceBackup || CSettings::GetObject()->General.bBackups) && !m_bBackupDone) {
		CString BakName;
...
void CFamiTrackerDoc::DeleteContents()
{
	...
	// Synchronized stop
	if (CSoundGen::GetObject()) {
		CSoundGen::GetObject()->StopPlayer();
		CSoundGen::GetObject()->WaitForStop();
	}
	CMIDI::GetObject()->ResetOutput();
...
void CFamiTrackerDoc::CreateEmpty()
{
	...
	CSoundGen::GetObject()->DocumentPropertiesChanged(this);
}
...
{
	if (l <= CSettings::GetObject()->Version.iErrorLevel && !Cond) {
...
void CFamiTrackerDoc::ReadBlock_DetuneTables(CDocumentFile *pDocFile, const int Version)
{
	...
	const CString CHIP_STR[6] = { _T("NTSC"), _T("PAL"), _T("Saw"), _T("VRC7"), _T("FDS"), _T("N163") };
	...
	for (int i = 0; i < Count; i++) {
		...
		try {
			...
			e->AppendError("At %s detune table,", CHIP_STR[Chip]);
			throw;
		}
	}
}
...
void CFamiTrackerDoc::ReadBlock_Bookmarks(CDocumentFile *pDocFile, const int Version)
{
	...
	for (int i = 0; i < Count; i++) {
		...
		pMark->m_sName = std::string(CT2A(pDocFile->ReadString()));
...
bool CFamiTrackerDoc::ImportDetune(CFamiTrackerDoc *pImported)		// // //
{
	...
	CSoundGen::GetObject()->LoadMachineSettings();		// // //
	return true;
}
...
{
	CInstrument *pInst = CChannelMap::GetObject()->GetChipInstrument(ChipType);
	if (!pInst) {
#ifdef _DEBUG
...
{ 
	if (CSettings::GetObject()->General.bShowSkippedRows)		// // //
...
void CFamiTrackerDoc::SetupChannels(unsigned char Chip)
{
	...
	CSoundGen::GetObject()->RegisterChannels(Chip, this); 
...
void CFamiTrackerDoc::ApplyExpansionChip()
{
	...
	CSoundGen::GetObject()->SelectChip(m_iExpansionChip);
