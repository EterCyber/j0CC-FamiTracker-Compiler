...
void CCompiler::ExportNSFE(LPCTSTR lpszFileName, int MachineType)		// // //
{
	...
	OutputFile.Write((char*)((PCSTR)CT2A(str)), str.GetLength() + 1);
	str.ReleaseBuffer();

	for (unsigned int i = 0; i < m_pDocument->GetTrackCount(); i++) {
		iTimeSize += 4;
		iTlblSize += strlen(CT2A(m_pDocument->GetTrackTitle(i))) + 1;
	}
	...
	for (unsigned int i = 0; i < m_pDocument->GetTrackCount(); i++) {
		OutputFile.Write(m_pDocument->GetTrackTitle(i), strlen(CT2A(m_pDocument->GetTrackTitle(i))) + 1);
	}
...
void CCompiler::PatchVibratoTable(char *pDriver) const
{
	...
	const CSoundGen *pSoundGen = CSoundGen::GetObject();
...
void CCompiler::StoreGrooves()
{
	...
	Print(_T(" * Grooves used: %i (%i bytes)\n"), Count, Size);
}
